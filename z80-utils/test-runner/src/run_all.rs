...
fn add_clang_filtered(
    ...
    suites.push(SuiteDef {
        ...
        runner: Box::new(move |paths, state, idx| {
            let config = ClangConfig { target, opt_levels: opts, fast_math, omit_fp, inline_runtime: false, static_stack: false, verify: false, diff_opt: false, pattern };
    ...
    suites.push(SuiteDef {
        ...
        runner: Box::new(move |paths, state, idx| {
            let config = ClangConfig {
                ...
                inline_runtime: true, static_stack: false, verify: false, diff_opt: false, pattern: None,
            };
            let test_dir = paths.clang_test_dir();
            let tests = crate::suite::discover_tests(&test_dir, "test_", "c");
