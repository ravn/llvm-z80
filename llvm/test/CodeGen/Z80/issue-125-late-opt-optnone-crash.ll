...
;   static uint16_t bug125(uint16_t x) {
;       uint16_t slot = x;
;       return callee(slot);
;   }
; clang -O0 marks functions optnone; two BSS allocas + store/load/call.
; Under +static-frame, Z80LateOptimization crashed (SEGV) on this shape.
