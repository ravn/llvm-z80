...
; to the natural width.  The walk previously bailed at the first non-Instruction
; non-Constant operand, which made it give up whenever the expression reached a
...
; final `trunc i16 ... to i8`.  Without narrowing through the argument the target
; emits a 16-bit shift/mask/or dance instead of native 8-bit ops.
;
; ravn/llvm-z80#158: accept an Argument as a narrowable leaf and emit a single
; explicit trunc of the argument at function entry (dominating every narrowed
; use).  Pre-fix this function is left entirely at i16; the i8 ops below are the
; regression signal.
