...
; ===== And-mask outside-user path (v2) =====
;
; `(and X, Const)` where Const fits in the narrow width and X is in-graph
; is sound to narrow REGARDLESS of X's KnownBits — the mask discards X's
; high bits unconditionally.  Rewritten as
; `(zext (and Xnarrow, ConstTrunc) to OrigTy)` so downstream consumers
; (stores, icmps on the AND result, calls) still see OrigTy.

; Unproven graph value, and-mask outside user that fits i8: must STILL
; narrow.  The store keeps OrigTy via the zext wrapper around the
; narrowed and.
...
; And-mask whose constant does NOT fit i8 (mask = 511): must NOT narrow.
define i8 @andmask_const_too_wide(i16 %x, ptr %p) {
; CHECK-LABEL: @andmask_const_too_wide(
; CHECK: add i16
; CHECK: and i16
; CHECK: store i16
  %t = add i16 %x, 1
...
; Mixed escapes where BOTH are admissible: icmp is on the constant-Other
; path with proven graph value (and 127 + 1, max 128 fits i8), and the
; and-mask is unconditionally sound.  Graph narrows.
define i8 @mixed_andmask_icmp_proven(i16 %x, ptr %p) {
; CHECK-LABEL: @mixed_andmask_icmp_proven(
; CHECK: add i8
; CHECK: and i8
; CHECK: icmp ult i8
  %m = and i16 %x, 127
