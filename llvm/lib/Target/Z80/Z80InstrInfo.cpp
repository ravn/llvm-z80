...
static bool aIsDeadInto(const MachineInstr &MI, const TargetRegisterInfo *TRI) {
  ...
  // A flag-only AND A/SBC A,A is safe to mark undef only when no value has
  // been defined for A on the path into this instruction.  Looking backward
  // from the instruction avoids treating a later RET or an unrelated
  // subregister use as evidence that the incoming accumulator is live.
  for (auto I = MBB.begin(); I != MBB.end() && &*I != &MI; ++I)
    if (I->definesRegister(Z80::A, TRI))
      return false;
  return !MBB.isLiveIn(Z80::A);
}
  ...
  case Z80::RET_CLEANUP: {
    ...
    if (Amount == 0) {
      ...
      if (HLDead && Amount >= 2) {
        ...
        // Z80 large: keep HL intact for an HL/DE return and use IY as the
        // temporary stack-address register.
        BuildMI(MBB, MI, DL, get(Z80::POP_BC));
        if (RetBits != 32) {
