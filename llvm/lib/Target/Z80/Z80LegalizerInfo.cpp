...
static cl::opt<bool> UseSDCCCall0ForF32Libcalls(
    ...
    cl::desc("Emit the f32 arithmetic/compare/conversion libcalls "
             "(__addsf3/__subsf3/__mulsf3/__divsf3, __cmpsf2/__gtsf2/__gesf2/"
             "__unordsf2, __fixsfsi/__fixunssfsi/__floatsisf/__floatunsisf) "
             "with CallingConv::Z80_SDCCCall0 instead of the default C ABI, "
             "so they alias z88dk math32's sdcccall(0) wrappers with zero "
             "glue code (ravn/llvm-z80#277). Only safe when linking against "
             "z88dk math32, NOT the ELF path's own compiler-rt float runtime, "
             "which expects the default ABI."));
...
Z80LegalizerInfo::Z80LegalizerInfo(const Z80Subtarget &STI) {
  ...
  // P2 (port I/O) pointer constants arise when the combiner folds
  // G_INTTOPTR(G_CONSTANT n) for a constant port into a pointer-typed constant;
  // like P0 it is a 16-bit pointer, so it is legal here.
  ...
  // f32↔i32 is custom (selects the sdcccall(0) math32 bridge or the
  ...
  // RuntimeLibcallsInfo (a table this backend does not customize), not a value
  // we can gate locally; custom legalization reuses the same
...
bool Z80LegalizerInfo::legalizeIntrinsic(LegalizerHelper &Helper,
  ...
  case Intrinsic::z80_pattern_fill: {
    ...
    // Emit the seed store FIRST, then set up the LDIR register triple
    // (HL=src, DE=dst+K, BC=len) immediately before the LDIR.  A K>=2 seed
    // store materialises the pattern in DE (`ld de,pattern; ld (hl),e; ...`),
    // so setting DE=dst+K before the seed leaves DE holding the pattern (not
    // the destination) at the LDIR -- a miscompile the pre-PR#40 scheduler
    // happened to avoid but PR #40's regalloc/scheduling exposed
    // (test_205_fill_word/_dword returned garbage).  Seeding first keeps the
    // physical-register triple contiguous and unclobbered.  Cost: +1 byte
    // after ZX0 on cpnos's IVT fill (byte-adjacency only); functionally
    // identical -- the seed lands in memory before the LDIR reads it.
    emitSeedAt(DstPtr);
...
bool Z80LegalizerInfo::legalizeCustom(LegalizerHelper &Helper, MachineInstr &MI,
  ...
  case TargetOpcode::G_FDIV: {
    ...
    // sdcccall(0) gate (ravn/llvm-z80 #277): z88dk math32's wrappers use the
    // sccz80/SmallC register convention (stack args, 32-bit result in DE:HL),
    // which is exactly clang's sdcccall(0). Gated because the ELF path's own
    // compiler-rt __addsf3 et al. expect the default C ABI (sdcccall(1)).
  ...
  case TargetOpcode::G_FCMP: {
    ...
    // z88dk math32's compare bridge is written for sdcccall(0); the ELF path's
    // own compiler-rt __cmpsf2/__gtsf2/__gesf2/__unordsf2 expect the default C
    // ABI. See -z80-float-sdcccall0's cl::opt comment above.
  ...
  case TargetOpcode::G_UITOFP: {
    ...
    // arithmetic/compare libcalls (ravn/llvm-z80 #277): z88dk's math32 bridge
    // aliases are written for sdcccall(0); the ELF path's own compiler-rt
