...
| **memset-pattern-hook** (2026-06-09) | TTI hook `shouldExpandExperimentalMemSetPattern` to let backends claim `llvm.experimental.memset.pattern` instead of letting `PreISelIntrinsicLowering` expand it to libcall-or-loop | POC on `main` (commit `6839ebc`); RFC body ready in `tasks/upstream-memset-pattern-issue-body.md`; **awaiting per-filing go-ahead** | Generic LLVM hook (~30 LOC); default `true` preserves all-backend behaviour; motivated by Z80 LDIR seed+propagate.  Pairs with the `Z80PatternFillRecognize` fork-local recogniser (whose body is target-agnostic — eventual retirement is via adding K-pattern recognition to upstream `LoopIdiomRecognize`, separate / much larger upstream change; section 7.2/7.3 of the design doc explains why we don't bid for `LoopIdiomRecognize` extensibility now).  Design: `tasks/design-upstream-memset-pattern-target-hook-2026-06-09.md`. |
...
| **#178** | Pseudos with implicit physreg outputs break rematerialization | Lever exhausted (session 73s) — revisit post-#112 | ROOT-CAUSED then FIXED-AND-MEASURED (`session73s-issue178-add16-tied-rootcause.md`). The tied `ADD16_tied` miscompiled because the RegisterCoalescer assigns the tied def an out-of-HLI-class physreg (BC) in the base-reuse shape; the BC-fallback's undeclared HL clobber corrupts unrelated values. Built the non-tied **`ADD16_acc`** (no tie -> no coalescer trap): correctness SOLVED (lit `add16-acc.ll`), but +219 B / +9.8% on AES09 and remat never fires (FLAGS-clobber blocks the rematerializer — byte-identical with/without `isReMaterializable`) + HL-pinning under the 3-pair file. Kept default-OFF (`-z80-add16-acc`). The remat attack is exhausted for the IX/IY-reserved allocator; `ADD16_acc`'s HLI class covers IX/IY, so revisit after **#112**. NB: a coalescer class-clamp fix is also in scope (fork-local, NOT an upstream PR) — just not the path taken. #166 inherits. |
...
| **#112 / #38** | Un-reserve IX/IY (more allocatable pairs) | In progress (session 73s) — dominant bug FIXED, 1 residual | Encoder opcode-0 crash FIXED (operand-class discipline). Dominant runtime miscompile FIXED: `LEA_IX_FI` had no IY case -> `llvm_unreachable` = silent no-op in Release -> IY undefined; added IY cases (commit `27be55e2569d`) + hidden `-z80-unreserve-iy` flag (default off) + lit `lea-fi-iy-112.ll`. Test-runner IY-on 622/85/76 -> **684/42/57** (baseline 690/37/56) — 63 of 70 regressions cleared. RESIDUAL (6 tests, test_04_i32_bitwise/test_40_hash_crc): byte-decomposition of an IY-allocated 16-bit value yields **undocumented IYH/IYL** half-ops. PERVASIVE, not one site — tried constraining `G_UNMERGE_VALUES` source to GR16NoIR, reverted (8-bit ALU folds bypass it; 40+ sub-register extraction sites). Crux: IY's byte halves are undocumented (why it was reserved). Splits into: pure-16-bit IY values (pointers/`ADD IY,rr`/load-store) safe today via the LEA_IX_FI fix; byte-decomposed values (i32 chunks, byte-wise logic) need EITHER **(A) +undocumented in production** (real Z80+MAME run IYH/IYL fine; only `feedback_no_undocumented_default` blocks it — cheapest, needs user policy sign-off) OR **(B) a regalloc constraint keeping IY off byte-accessed vregs** (large/fragile). Prize: −33 B/−1.5% AES09 + 3-pair pressure relief (#27/#110/#115/#178), real under either path. Strategic (A)-vs-(B) decision for the user. Production byte-identical (flag off). **UPDATE (session 73s):** user chose (B); built a pre-RA class-narrowing pass -- it removed the undoc ops (0 refs) but tests STILL failed + broke test_30 (net 10 vs baseline 6) -> REVERTED. Diagnosis corrected: undoc was a co-symptom; the residual is a deeper wrong-value regalloc miscompile (IY holding byte-decomposed values), the **same RegisterCoalescer out-of-class root as #178**. So #112's residual and #178 are in the same regalloc-correctness family. **Correction:** ticks DOES execute undocumented IXH/IYH (canixh() true for Z80), so the IY-on failures are REAL wrong-value miscompiles, not an emulation gap -> (A) +undocumented would NOT fix the residual either. The residual is a genuine regalloc/coalescer bug; fixing it needs an **assertions-enabled LLVM build** (`-debug-only=regalloc`) to trace -- a fresh focused session (current Release build has assertions off). See `session73s-issue112-iy-unreserve-scope.md`. |
