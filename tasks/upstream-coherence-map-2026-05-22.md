...
| **#112 / #38** | Un-reserve IX/IY (more allocatable pairs) | Reframed (session 73s) — blocker relocated | The session-40 "387 FATAL opcode-0 encoder crashes" are FIXED (GR16NoIR/GR16_BCDE operand discipline on all ~25 decomposing expansions; IY-allocatable lit = 110+5, no FATALs). What remains is a single **runtime IY-allocation miscompile** (AES runs away, 300M vs 11.5M tstates; no undoc leakage) — likely the #14 IY-copy/'y'-crash class. Prize: **−33 B / −1.5% AES09** + relieves 3-pair pressure (#27/#110/#115/#178). NOT "Phase 3 cost-model work" — it's a focused miscompile-isolation drill. See `session73s-issue112-iy-unreserve-scope.md`. |
