...
| **#178** | Pseudos with implicit physreg outputs break rematerialization | Blocked on RegisterCoalescer (root-caused, session 73s) | ROOT-CAUSED (5-line repro, `session73s-issue178-add16-tied-rootcause.md`): in the base-reuse shape the RegisterCoalescer merges the GR16 base, the HLI accumulator copy, and the HLI-classed `ADD16_tied` tied-def into one interval and keeps the base's physreg (BC) — OUTSIDE the tied def's HLI class (the join widens to GR16 instead of clamping to the HLI subclass). dst=BC then hits the BC-fallback whose undeclared HL clobber corrupts unrelated values. Obvious patches fail (HL in Defs => out-of-registers; narrower class => coalescer escapes anyway). Fix = coalescer tied-def class-clamp (generic LLVM, out of scope while upstream paused) OR non-tied remat model. #178 + #166 both gate on this. |
